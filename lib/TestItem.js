...
TestItem.prototype.run = function(callback) {
  if (this.isInProgress) {
    console.warn(new Date().toJSON() + ' fh-health: Called run method for test "' + this.getDescription() + '" while it was already running. Will not run again until first run is complete.');
  } else {
    var self = this,
      startTime = Date.now(),
      timedOut = false;
